#!/bin/bash
#=============================================================================
# BBR v3 Финальный оптимизационный скрипт - Ultimate Edition
# Функциональность: Сочетает в себе стабильность официального ядра XanMod
#                   и профессиональную настройку алгоритмов очередей.
# Особенности: Двойная оптимизация: безопасность + производительность.
#=============================================================================
# Правила управления версиями:
# 1. При выпуске новой версии изменяйте SCRIPT_VERSION и обновляйте
#    журнал изменений (сохраняйте последние 5 записей).
# 2. При временном исправлении/без выпуска версии изменяйте только
#    SCRIPT_LAST_UPDATE, чтобы быстро идентифицировать обновленный скрипт.
#=============================================================================
# v5.4.4 Обновление: В главный список меню 33 добавлены столбцы «День сброса»
#    и «Примечание» (в ответ на issue #22) — если есть reset_day,
#    отображается "Каждое X-е число месяца", иначе "Нет сброса";
#    примечание, если пустое, отображается "-"; также исправлено отображение
#    пустой даты истечения срока (jq // не обрабатывает пустые строки,
#    из-за чего столбец даты для бессрочных портов всегда был пустым,
#    теперь отображается "Бессрочно") (от Eric86777)
# v5.4.3 Обновление: В меню 33 исправлено 7 проблем — ① добавлено
#    автоматическое восстановление после загрузки (ptm-boot-restore.sh +
#    systemd-сервис + снепшот трафика), после перезагрузки автоматически
#    восстанавливаются счетчики nftables/квоты/tc-ограничения и
#    повторно блокируются просроченные порты (ранее после перезагрузки
#    всё сбрасывалось); ② в ежедневную проверку добавлены уведомления
#    по электронной почте при достижении порогов 80%/95% квоты
#    (ранее полностью отсутствовали); ③ добавлено предупреждение
#    за 3 дня до истечения срока (ранее полностью отсутствовало);
#    ④ порты с просрочкой ≥3 дней теперь автоматически полностью
#    очищаются и удаляются через cron (ранее только логировались,
#    но не очищались); ⑤ tc_remove_limit изменено на точное удаление
#    по порту (ранее использовалось tc filter del без условий,
#    что очищало все правила ограничения скорости на этом интерфейсе);
#    ⑥ исправлено дублирование правил квоты; ⑦ удален мертвый код
#    из меню 33; ⑧ исправлено отображение пользовательского порта
#    в Sub2API (меню 32) — регулярное выражение для извлечения порта
#    не соответствовало официальному формату Environment=SERVER_PORT=,
#    из-за чего на странице развертывания/статуса всегда отображался
#    порт 8282, а функция изменения порта молча не работала;
#    теперь за основу берется файл systemd-сервиса, а устаревший
#    файл порта автоматически исправляется (от Eric86777)
# v5.4.2 Обновление: Меню 33 «Быстрое открытие порта» переработано
#    в соответствии с оригинальной версией dog в виде многошагового
#    мастера (порт → режим тарификации → квота → примечание →
#    день сброса → срок аренды → email), восстановлена функция
#    "Объединить порты в группу"; ввод квоты/пропускной способности
#    изменен на "0=без ограничений" (единицы MB/GB/T, Kbps/Mbps/Gbps);
#    исправлены номера вариантов режимов тарификации (2=только исходящий,
#    3=CN Premium, ранее были перепутаны); управление пропускной
#    способностью/квотой/сроком аренды теперь осуществляется
#    по номерам портов (от Eric86777)
# v5.4.1 Обновление: Структура меню 33 «Управление учетом трафика
#    и сроками действия портов» приведена в соответствие с группировкой
#    оригинального dog (1 Добавление/Удаление, 2 Настройка ограничений
#    [пропускная способность/квота/срок], 3 Управление сбросом
#    [день сброса/немедленный сброс], 4 Уведомления, 5 Диагностика,
#    99 Удаление); исправлено дублирование цифр "защищаемых портов"
#    в строке состояния главного меню (от Eric86777)
# v5.4.0 Обновление: Упрощен набор инструментов AI-прокси
#    (удалены модули Antigravity/OpenClaw/CLIProxyAPI/Codex Console/OAI2);
#    добавлено меню 33 «Управление учетом трафика и сроками действия портов»
#    (счетчики nftables/квоты + ограничение скорости через tc + автоматическая
#    остановка по истечению срока + опциональные уведомления по email)
#    (от Eric86777)

SCRIPT_VERSION="5.4.4"
SCRIPT_LAST_UPDATE="Исправление текста: номер CF Tunnel в README изменен с 32-7 на 32-6; метка главного меню 32 приведена в соответствие с фактическим подменю"
#=============================================================================

#=============================================================================
# 📋 Рекомендуемые схемы конфигурации (основаны на реальных тестах)
#=============================================================================
#
# 💡 Тестовая среда: Протестировано на более чем десятке различных серверов,
#    включая несколько узлов Kuxueyun Beijing 9929.
#
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#
# ⭐ Основной вариант (рекомендуемый):
#    Шаг 1 → Выполнить пункт меню 1: Установка ядра BBR v3
#    Шаг 2 → Выполнить пункт меню 3: Оптимизация BBR для прямого/конечного подключения
#            (Интеллектуальное определение пропускной способности)
#            Выберите подпункт 1 для автоматического определения.
#    Шаг 3 → Выполнить пункт меню 6: Исправление таймаута первого подключения Realm
#            (если используется Realm).
#
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#
# 🔧 Запасной вариант (резервный):
#    Шаг 1 → Выполнить пункт меню 1: Установка ядра BBR v3
#    Шаг 2 → Выполнить пункт меню 5: Настройка CAKE с форума NS
#    Шаг 3 → Выполнить пункт меню 6: Оптимизация параметров ядра
#            в высокопроизводительном режиме от Kejilion
...
# Определение цветов (имена переменных на китайском сохранены
# для совместимости с существующим кодом)
gl_hong='\033[31m'      # Красный
gl_lv='\033[32m'        # Зеленый
gl_huang='\033[33m'     # Желтый
gl_bai='\033[0m'        # Сброс
gl_kjlan='\033[96m'     # Ярко-голубой
gl_zi='\033[35m'        # Фиолетовый
gl_hui='\033[90m'       # Серый
...
show_main_menu() {
    ...
    echo -e "${gl_zi}║ $(format_fixed_width "BBR v3 Финальный оптимизационный скрипт - Ultimate Edition" $((inner - 2))) ║${gl_bai}"
    echo -e "${gl_zi}║ $(format_fixed_width "Версия ${SCRIPT_VERSION}" $((inner - 2))) ║${gl_bai}"
    if [ -n "$SCRIPT_LAST_UPDATE" ]; then
        echo -e "${gl_zi}║ ${gl_huang}$(format_fixed_width "Обновление: ${SCRIPT_LAST_UPDATE}" $((inner - 2)))${gl_zi} ║${gl_bai}"
    fi
    echo -e "${gl_zi}╚$(printf '═%.0s' $(seq 1 $inner))╝${gl_bai}"
    echo ""
    echo -e "${gl_kjlan}━━━━━━━━━━━━ Основные функции ━━━━━━━━━━━━${gl_bai}"
    echo -e "${gl_kjlan}[Управление ядром]${gl_bai}"
    echo "1. Установить/Обновить ядро XanMod + BBR v3 ⭐ Рекомендуется"
    echo "2. Удалить ядро XanMod"
    echo ""
    echo -e "${gl_kjlan}[Оптимизация BBR/Сети]${gl_bai}"
    echo "3. Оптимизация BBR для прямого/конечного подключения (Интеллектуальное определение пропускной способности) ⭐ Рекомендуется"
    echo -e "4. ${gl_hui}Удалено (функция 3 теперь покрывает интеллектуальное обнаружение MTU)${gl_bai}"
    echo "5. NS-форум - Очистка DNS (Защита от подмены/Настройка DHCP)"
    echo "6. Исправление таймаута первого подключения Realm ⭐ Рекомендуется"
    echo ""
    echo -e "${gl_kjlan}━━━━━━━━━━━━ Настройка системы ━━━━━━━━━━━${gl_bai}"
    echo "7. Установить приоритет IPv4/IPv6"
    echo "8. Управление IPv6 (Временное/Постоянное отключение/Отмена)"
    echo "9. Установить временный SOCKS5-прокси"
    echo "10. Управление виртуальной памятью"
    echo "11. Показать детальное состояние системы"
    echo ""
    echo -e "${gl_kjlan}━━━━━━━━━━ Развертывание прокси ━━━━━━━━━━${gl_bai}"
    echo "12. Протокол Snell от Xingchendahai ⭐ Рекомендуется"
    echo "13. Мультипротокольный Xray от Xingchendahai ⭐ Рекомендуется"
    echo "14. Запретить прямое подключение портов из материкового Китая"
    echo "15. Развернуть SOCKS5-прокси одним нажатием"
    echo "16. Управление несколькими экземплярами Sub-Store"
    echo "17. Обратный прокси одним нажатием ⭐ Рекомендуется"
    echo ""
    echo -e "${gl_kjlan}━━━━━━━━━━━ Тестирование и диагностика ━━━━━━━━━━━${gl_bai}"
    echo -e "${gl_kjlan}[Проверка качества IP]${gl_bai}"
    echo "18. Проверка качества IP (IPv4+IPv6)"
    echo "19. Проверка качества IP (только IPv4) ⭐ Рекомендуется"
    echo ""
    echo -e "${gl_kjlan}[Сетевые тесты]${gl_bai}"
    echo "20. Тест пропускной способности сервера"
    echo "21. iperf3 однопоточный тест"
    echo "22. Тест скорости международного соединения ⭐ Рекомендуется"
    echo "23. Проверка качества сетевой задержки ⭐ Рекомендуется"
    echo "24. Трассировка маршрута обратного пути (CN-сети) ⭐ Рекомендуется"
    echo ""
    echo -e "${gl_kjlan}[Проверка стриминга/AI]${gl_bai}"
    echo "25. Проверка разблокировки стриминговых/AI-сервисов ⭐ Рекомендуется"
    echo "26. Быстрая проверка NQ ⭐ Рекомендуется"
    echo ""
    echo -e "${gl_kjlan}━━━━━━━━━━ Сторонние инструменты ━━━━━━━━━━${gl_bai}"
    echo "27. zywe_realm скрипт для перенаправления"
    echo "28. Скрипт Fscarmen для sing-box"
    echo "29. Скрипт Kejilion"
    echo "30. Настройка CAKE с форума NS"
    echo "31. Высокопроизводительный режим Kejilion"
    echo ""
    echo -e "${gl_kjlan}━━━━━━━━━ AI Прокси-сервисы ━━━━━━━━━${gl_bai}"
    echo "32. Набор инструментов AI-прокси ▶ (WebUI/CRS/Fuclaude/Sub2API/Caddy/CF-Tunnel и др.) ⭐ Рекомендуется"
    echo ""
    echo -e "${gl_kjlan}━━━━━━━━━ Управление трафиком и портами ━━━━━━━━━${gl_bai}"
    echo "33. Учет трафика и управление сроками действия портов 🆕"
    echo ""
    echo -e "${gl_kjlan}━━━━━━━━━ Оптимизация в один клик ━━━━━━━━━${gl_bai}"
    echo "66. ⭐ Полная автоматическая оптимизация (BBR v3 + Настройка сети)"
    echo ""
    echo -e "${gl_hong}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
    echo -e "${gl_hong}99. Полное удаление скрипта (удалить всё)${gl_bai}"
    echo ""
    ...
    case $choice in
        1)
            if [ $is_installed -eq 0 ]; then
                ...
                # Функция обновления уже содержит интерактив по перезагрузке
            else
        ...
        2)
            if [ $is_installed -eq 0 ]; then
                ...
                echo -e "${gl_huang}Ядро XanMod не обнаружено, удаление не требуется${gl_bai}"
                break_end
            fi
            ;;
        3)
            ...
            ;;
        4)
            echo -e "${gl_huang}⚠️ Функция 4 удалена в версии v4.9.2, функция 3 (tcp_mtu_probing) теперь покрывает интеллектуальное обнаружение MTU${gl_bai}"
            break_end
            ;;
        5)
            dns_purify_and_harden
...
update_xanmod_kernel() {
    clear
    echo -e "${gl_kjlan}=== Обновление ядра XanMod ===${gl_bai}"
    echo "------------------------------------------------"
    
    # Получение текущей версии ядра
    local current_kernel=$(uname -r)
    echo -e "Текущая версия ядра: ${gl_huang}${current_kernel}${gl_bai}"
    echo ""
    
    # Определение архитектуры CPU
    local cpu_arch=$(uname -m)
    
    # Подсказка для ARM архитектуры
    if [ "$cpu_arch" = "aarch64" ]; then
        echo -e "${gl_huang}Автоматическое обновление для ARM64 временно не поддерживается${gl_bai}"
        echo "Рекомендуется удалить и переустановить для получения последней версии"
        break_end
        return 1
    fi
    
    # Процесс обновления для архитектуры x86_64
    echo "Проверка доступных обновлений..."
    
    local xanmod_repo_file="/etc/apt/sources.list.d/xanmod-release.list"

    # Добавление/исправление репозитория XanMod
    if [ ! -f "$xanmod_repo_file" ] || grep -qE 'deb\.xanmod\.org[[:space:]]+releases[[:space:]]+' "$xanmod_repo_file" 2>/dev/null; then
        ...
        if [ "$gpg_ok" = false ]; then
            echo -e "${gl_hong}Ошибка: Не удалось импортировать GPG-ключ${gl_bai}"
            break_end
            return 1
        fi

        # Добавление репозитория (используется codename системы)
    ...
    if ! apt-get update > /dev/null 2>&1; then
        echo -e "${gl_huang}⚠️  apt-get update частично не удался, пробуем продолжить...${gl_bai}"
    fi

    # Проверка установленных пакетов XanMod (фильтр ^ii, исключаем остатки удаленных)
    local installed_packages=$(dpkg -l | grep -E '^ii\s+linux-.*xanmod' | awk '{print $2}')
    
    if [ -z "$installed_packages" ]; then
        echo -e "${gl_hong}Ошибка: Не обнаружено установленных пакетов ядра XanMod${gl_bai}"
        break_end
        return 1
    fi
    
    echo -e "Установленные пакеты ядра:"
    ...
    if [ -z "$upgradable" ]; then
        ...
        # Получение последней установленной версии XanMod
        ...
        if [ $running_latest -eq 1 ]; then
            echo -e "${gl_lv}✅ Текущее работающее ядро уже является последней версией!${gl_bai}"
        else
            echo -e "${gl_lv}✅ Пакеты ядра XanMod уже обновлены, но текущее работающее ядро еще не переключено!${gl_bai}"
            echo -e "  Работает: ${gl_hong}${current_kernel}${gl_bai}"
            if [ -n "$latest_installed" ]; then
                echo -e "  Последняя установленная: ${gl_lv}${latest_installed}${gl_bai}"
            else
                echo -e "  ${gl_huang}Подсказка: Не удалось определить последнюю установленную версию ядра, перезагрузитесь и проверьте снова${gl_bai}"
            fi
            echo -e "  ${gl_huang}Пожалуйста, перезагрузите систему (reboot) для переключения на последнее ядро${gl_bai}"
        fi
        echo ""

        echo -e "${gl_kjlan}━━━━━━━━━━ Информация о CPU ━━━━━━━━━━${gl_bai}"
        echo -e "  Уровень архитектуры CPU: ${gl_lv}x86-64-v${cpu_level}${gl_bai}"
        echo -e "  Текущее работающее ядро: ${gl_lv}${current_kernel}${gl_bai}"
        if [ -n "$latest_installed" ] && [ $running_latest -ne 1 ]; then
            echo -e "  Последнее установленное ядро: ${gl_lv}${latest_installed}${gl_bai}"
        fi
        if [ $running_latest -eq 1 ]; then
            echo -e "  ${gl_huang}Примечание: Ваш CPU поддерживает максимум v${cpu_level}, уже запущено последнее ядро этого уровня${gl_bai}"
        else
            echo -e "  ${gl_huang}Примечание: Ваш CPU поддерживает максимум v${cpu_level}, последнее ядро установлено, вступит в силу после перезагрузки${gl_bai}"
        fi
        echo -e "  ${gl_huang}Ядра разных уровней (v1-v4) могут обновляться с разной скоростью, в зависимости от официального репозитория XanMod${gl_bai}"
        echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"

        rm -f "$xanmod_repo_file"
        echo -e "${gl_lv}Репозиторий XanMod очищен (при необходимости обновления снова запустите пункт 1)${gl_bai}"
        break_end
        return 0
    fi
    
    echo -e "${gl_huang}Обнаружены доступные обновления:${gl_bai}"
    echo "$upgradable"
    echo ""
    
    read -e -p "Выполнить обновление ядра XanMod? (Y/N): " confirm
    ...
    case "$confirm" in
        [Yy])
            ...
            if [ $? -eq 0 ]; then
                echo ""
                echo -e "${gl_lv}✅ Ядро XanMod успешно обновлено!${gl_bai}"
                echo -e "${gl_huang}⚠️  Пожалуйста, перезагрузите систему для загрузки нового ядра${gl_bai}"
                echo ""
                local cpu_level
                cpu_level=$(echo "$installed_packages" | sed -nE 's/.*x64v([1-4]).*/\1/p' | head -1)
                [ -z "$cpu_level" ] && cpu_level="3"
                ...
                if [ -n "$latest_installed" ]; then
                    ...
                    echo -e "  Обновленный пакет ядра: ${gl_lv}$(echo "$installed_packages" | head -1)${gl_bai}"
                fi
                echo -e "  ${gl_huang}Примечание: Ваш CPU поддерживает максимум v${cpu_level}, обновлено до последнего ядра этого уровня${gl_bai}"
                echo -e "  ${gl_huang}Ядра разных уровней (v1-v4) могут обновляться с разной скоростью, в зависимости от официального репозитория XanMod${gl_bai}"
                echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
                echo ""
                echo -e "${gl_kjlan}Последующие обновления: снова запустите пункт 1 для проверки и установки последней версии${gl_bai}"
                ...
                echo ""
                echo -e "${gl_hong}❌ Обновление ядра не удалось${gl_bai}"
                break_end
                return 1
            fi
            ;;
